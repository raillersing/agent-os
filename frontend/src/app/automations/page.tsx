...
export default function Automations() {
  ...
  const refresh = async () => {
    setLoading(true)
    try {
      const currentWorkspaceId: string | null = (await api.listWorkspaces())[0]?.id || null
      ...
      const automationData = await api.listAutomations(currentWorkspaceId)
  ...
  useEffect(() => { refresh() }, [])
  const create = async () => {
    if (!workspaceId) { setError('Create a workspace before creating an automation.'); return }
    try {
      const automation = await api.createAutomation({ workspace_id: workspaceId, name: 'New scheduled brief', description: 'A bounded, reviewable scheduled workflow.', trigger_type: 'schedule', trigger_config: { cron: '0 8 * * 1' }, steps: [{ name: 'Collect' }, { name: 'Prepare' }, { name: 'Review' }] })
      setAutomations((current) => [automation, ...current])
      setError('')
    } catch (cause) { setError(cause instanceof Error ? cause.message : 'Automation could not be saved.') }
  }
